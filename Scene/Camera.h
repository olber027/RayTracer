...
namespace scene
{
    ...
    public:
        ...
        void fromJson(const nlohmann::json& camera_json)
        {
            ...
            std::vector<double> zAxis{0, 0, 1.0};
            std::vector<double> yAxis{0, 1.0, 0};
            auto vector_to_string = [] (auto val) { return "[" + std::to_string(val[0]) + ", " + std::to_string(val[1]) + ", " + std::to_string(val[2]) + "]"; };

            try {
                position = camera_json.at("position").get<std::vector<double>>();
            ...
            try {
                zAxis = camera_json.at("z_axis").get<std::vector<double>>();
            } catch(std::exception& e) {
            ...
            try {
                yAxis = camera_json.at("y_axis").get<std::vector<double>>();
            } catch(std::exception& e) {
